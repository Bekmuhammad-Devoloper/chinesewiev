...
export default function LessonDetailClient({
  ...
  const [lesson] = useState<Lesson | null>(initialLesson);
  const [dataLoading, setDataLoading] = useState(false);

  useEffect(() => {
    if (!initialLesson.locked) return;
    let isAuth = false;
    try {
      ...
      if (session) {
        ...
        if (user && new Date(user.expiresAt) > new Date()) {
          if (user.course === slug && !user.lessonId) isAuth = true;
          if (user.course === slug && user.lessonId === lessonId) isAuth = true;
        }
      }
    } catch {}
    if (!isAuth) {
  ...
  }, [slug, lessonId, initialLesson.locked, router]);
